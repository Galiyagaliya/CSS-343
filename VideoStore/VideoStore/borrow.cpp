...
// read the line, get id and check for valid customer id
...
void Borrow::proccessTransaction(string line, CustomerManager& cus_mang,
                                 InventoryManager& inv_mang)
{
    ...
    if (checkValidCustomer(customer, customer_id))
...
// Parse  the line into small pices. Based on these pices, creates pointer to a movie and retives actual movie
// if retrieve is succefull, proccess borrowMovie()
...
// Check if  cusomter can borrow movie.
// Check the actual stock for a movie. Also, add error message if movie doesn't exist in inventory manager.
